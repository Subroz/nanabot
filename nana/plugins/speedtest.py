...
async def speedtestxyz_callback(client, query):
    if query.from_user.id in AdminSettings:
        ...
        if query.data == 'speedtest_image':
            ...
            _, message_id, chat_id, _ = struct.unpack(
                "<LLiq",
                base64.urlsafe_b64decode(query.inline_message_id+"="*(
                    len(
            ...
            chat_id = str(chat_id).replace('-1', '-1001')
            await asyncio.gather(
                app.delete_messages(int(chat_id), message_id),
                app.send_photo(int(chat_id), speedtest_image)
            )
